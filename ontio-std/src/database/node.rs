...
use crate::cmp::min;
use crate::abi::{Sink, Encoder, Decoder, Source};
use crate::abi::Error;
use crate::database;
                ...
                if !self.is_full() {
                    ...
                    loop {
                        ...
                        if self.keys.get((ind-1) as usize).unwrap().to_owned() > key {
                            ...
                            if self.children.len() ==0 || self.children.get(ind as usize).is_none()
                                || ind >= (self.children.len() as u32) {
                                self.children.insert(ind as usize, self.children[(ind-1) as usize].clone());
                            } else {
                                self.children[ind as usize] = self.children[(ind-1) as usize].clone();
                            }
                        }
                        ind -= 1;
                    }
                    self.keys[index as usize] = key;
                    if self.children.len() ==0 || self.children.get(index as usize).is_none()
