use super::types::{Address,U256,to_neo_bytes};
use super::abi::Sink;
use super::runtime;
use super::console;
use hex;
...
pub mod ont {
    ...
    pub fn transfer(version:u8, from: &Address, to: &Address, amount: U256) -> bool {
        super::transfer_inner(&ONT_CONTRACT_ADDRESS,"transfer", version, from, to, amount)
    }
    pub fn approve(version:u8, from: &Address, to: &Address, amount: U256) -> bool {
        super::transfer_inner(&ONT_CONTRACT_ADDRESS,"approve", version, from, to, amount)
    }
    pub fn balance_of(version:u8, address: &Address) -> U256 {
        super::balance_of(&ONT_CONTRACT_ADDRESS,version, &address)
    }
    pub fn allowance(version:u8, from:&Address, to: &Address) -> U256 {
        super::allowance(&ONT_CONTRACT_ADDRESS, version, from, to)
    }
    pub fn transfer_from(version:u8, sender:&Address, from: &Address, to: &Address, amount: U256) -> bool {
        super::transfer_from(&ONT_CONTRACT_ADDRESS,version, sender, from, to, amount)
    }
}
...
pub mod ong {
    ...
    pub fn transfer(version:u8, from: &Address, to: &Address, amount: U256) -> bool {
        super::transfer_inner(&ONG_CONTRACT_ADDRESS,"transfer", version, from, to, amount)
    }
    pub fn balance_of(version:u8, address: &Address) -> U256 {
        super::balance_of(&ONG_CONTRACT_ADDRESS,version, &address)
    }
    pub fn allowance(version:u8, from:&Address, to: &Address) -> U256 {
        super::allowance(&ONG_CONTRACT_ADDRESS, version, from, to)
    }
    pub fn transfer_from(version:u8, sender:&Address, from: &Address, to: &Address, amount: U256) -> bool {
        super::transfer_from(&ONG_CONTRACT_ADDRESS,version, sender, from, to, amount)
    }
}

pub(crate) fn transfer_inner(contract_address: &Address, function_name:&str, version:u8, from: &Address, to: &Address, amount: U256) -> bool {
    ...
    let da = sink.into();
    runtime::notify(hex::encode(da.clone()).as_bytes());
    console::debug(hex::encode(da.clone()).as_str());
